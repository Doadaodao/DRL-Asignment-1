...
# -------------------------------
def extract_features(obs, grid_size):
    """
    ...
    taxi_row_norm = taxi_row / (grid_size - 1)
    taxi_col_norm = taxi_col / (grid_size - 1)
    features = [taxi_row_norm, taxi_col_norm]
    
    for (s_row, s_col) in stations:
        diff_row = (s_row - taxi_row) / (grid_size - 1)
        diff_col = (s_col - taxi_col) / (grid_size - 1)
        manhattan_dist = (abs(s_row - taxi_row) + abs(s_col - taxi_col)) / (2 * (grid_size - 1))
...
def train():
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            features = extract_features(obs, grid_size)
